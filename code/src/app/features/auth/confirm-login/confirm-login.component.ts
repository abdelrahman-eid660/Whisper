  ...
  template: `
    <div class="glass-card p-8 space-y-6">
      ...
      <div class="text-center space-y-2">
        ...
        <h1 class="font-display text-2xl font-semibold text-[rgb(var(--color-text))]">Two-factor auth</h1>
...
export class ConfirmLoginComponent implements OnInit, OnDestroy {
  ...
  confirm(): void {
    ...
    this.authService.confirmLogin({ email: this.email(), otp: this.otp() }).subscribe({
      next: res => {
        const d = res.data;
        if (d?.access_Token && d?.refreash_Token) {
          this.authStore.setAuth(d.user, d.access_Token, d.refreash_Token);
  ...
  resend(): void {
    ...
    this.authService.resendConfirmLogin({ email: this.email() }).subscribe({
      next: () => {
